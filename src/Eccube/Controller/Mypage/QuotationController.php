...
class QuotationController extends AbstractController
{
     ...
     * お届け先一覧画面.
     *
     * @Route("/mypage/quotation", name="mypage_quotation", methods={"GET"})
     * @Template("Mypage/quotation.twig")
     */
     ...
     * お届け先編集画面.
     *
     * @Route("/mypage/quotation/new", name="mypage_quotation_new", methods={"GET", "POST"})
     * @Route("/mypage/quotation/{id}/edit", name="mypage_quotation_edit", requirements={"id" = "\d+"}, methods={"GET", "POST"})
     * @Template("Mypage/quotation_edit.twig")
     */
    public function edit(Request $request, $id = null)
    {
        ...
        // 正しい遷移かをチェック
        $allowedParents = [
            $this->generateUrl('mypage_quotation'),
        ...
        if (!in_array($parentPage, $allowedParents)) {
            ...
            $parentPage = $this->generateUrl('mypage_quotation');
        }
        ...
        $this->eventDispatcher->dispatch(EccubeEvents::FRONT_MYPAGE_QUOTATION_EDIT_INITIALIZE, $event);
        ...
        if ($form->isSubmitted() ){// && $form->isValid()) {
            ...
            $this->eventDispatcher->dispatch(EccubeEvents::FRONT_MYPAGE_QUOTATION_EDIT_COMPLETE, $event);
 
            return $this->redirect($this->generateUrl('mypage_quotation'));
     ...
     * お届け先を削除する.
     *
     * @Route("/mypage/quotation/{id}/delete", name="mypage_quotation_delete", methods={"DELETE"})
     */
    public function delete(Request $request, CustomerAddress $CustomerAddress)
    {
        ...
        $this->eventDispatcher->dispatch(EccubeEvents::FRONT_MYPAGE_QUOTATION_DELETE_COMPLETE, $event);
    ...
    function parser($url, $Customer, $request)
    {
        ...
        */
        ...
        //dump($otherElements);die;
        if ($otherElements !== false && $otherElements !==  null) {
            $items = preg_split('/\s+/', $otherElements->nodeValue, -1, PREG_SPLIT_NO_EMPTY);
            for ($i = 0; $i < count($items); $i++) {
     ...
     * お届け先を削除する.
     *
     * @Route("/mypage/quotation/upload", name="mypage_quotation_upload", methods={"GET", "POST"})
     * @Template("Mypage/quotation_upload.twig")
     */
    public function upload(Request $request): JsonResponse
    {
        ...
        $uploadDir = $this->getParameter('/eccube-mylocal/ec-cube/html/upload/quotation/bm');
